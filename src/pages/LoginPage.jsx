...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Capture the response data
      const data = await authService.login(payload);

      // 2. CHECK FOR FORCE PASSWORD CHANGE FLAG
      if (data.requirePasswordChange) {

        // --- ADD THIS: Save Token Temporarily ---
        // We need this token to authorize the /change-password API call next
        localStorage.setItem("user", JSON.stringify(data));

        // If your Axios setup reads from "token" key specifically:
  ...
  return (
    <Box sx={{
      ...
      bgcolor: '#f1f5f9', p: 2, width: '100vw',
    }}>
