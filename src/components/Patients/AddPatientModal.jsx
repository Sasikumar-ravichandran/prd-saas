...
import { useSelector } from 'react-redux'; // ADDED to get Clinic Type
import {
  ...
  Divider, Stack, RadioGroup, FormControlLabel, Radio, MenuItem, DialogTitle
} from '@mui/material';
...
/* import DocumentScannerIcon from '@mui/icons-material/DocumentScanner'; */ // Commented out AI icon

import { patientService } from '../../api/services/patientService';
import api from '../../api/services/api';
import { useToast } from '../../context/ToastContext';

// PHASE 4: DYNAMIC CLINIC DICTIONARIES
const CLINIC_DICTIONARIES = {
  Dental: {
    concerns: ['Tooth Pain', 'Cleaning', 'Braces', 'Implant', 'Cosmetic', 'General Checkup', 'Sensitivity'],
    conditions: ['None', 'Diabetes', 'BP (High/Low)', 'Heart Issue', 'Pregnancy', 'Asthma', 'Thyroid', 'Epilepsy']
  },
  Dermatology: {
    concerns: ['Acne', 'Hair Fall', 'Pigmentation', 'Skin Rash', 'Anti-Aging', 'Mole Check', 'Scar Removal'],
    conditions: ['None', 'Eczema', 'Psoriasis', 'Allergies', 'Diabetes', 'Thyroid', 'Pregnancy', 'PCOS']
  },
  General_Practice: {
    concerns: ['Fever', 'Cough/Cold', 'Body Ache', 'Fatigue', 'Stomach ache', 'Headache', 'Routine Checkup'],
    conditions: ['None', 'Diabetes', 'Hypertension', 'Asthma', 'Heart Disease', 'Thyroid', 'Kidney Issue']
  },
  Ophthalmology: {
    concerns: ['Blurry Vision', 'Eye Pain', 'Dry Eyes', 'Cataract Consult', 'Redness', 'Spectacle Check', 'Glaucoma'],
    conditions: ['None', 'Diabetes', 'Hypertension', 'Glaucoma History', 'Thyroid', 'Cataract']
  },
  Physiotherapy: {
    concerns: ['Back Pain', 'Neck Pain', 'Joint Stiffness', 'Sports Injury', 'Post-Op Rehab', 'Muscle Spasm', 'Posture'],
    conditions: ['None', 'Arthritis', 'Osteoporosis', 'Diabetes', 'Hypertension', 'Heart Disease', 'Pregnancy']
  }
};
...
export default function AddPatientModal({ open, onClose, onSubmit, initialData }) {
  ...
  // GET CLINIC TYPE FROM REDUX (Fallback to General Practice)
  const clinicType = useSelector((state) => state.auth?.user?.clinicType) || 'General_Practice';
  const activeDictionary = CLINIC_DICTIONARIES[clinicType] || CLINIC_DICTIONARIES['General_Practice'];
  ...
  return (
    ...
    >
      {/* HEADER */}
      <Box sx={{ px: { xs: 2, md: 3 }, py: 2, borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center', bgcolor: isEditing ? 'white' : '#f9fafb' }}>
        <Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, md: 2 } }}>
          {/* AI Scan Button Commented Out
          {!initialData && isEditing && (
            <Button variant="contained" component="label" disabled={isScanning} ... >
          ...
          */}
          {initialData && !isEditing && (
            <Button variant="outlined" startIcon={<EditIcon />} onClick={() => setIsEditing(true)} sx={{ mr: { xs: 0, md: 2 }, borderRadius: 2 }}>
              Edit
            </Button>
          )}
      ...
      <DialogContent sx={{ p: 0 }}>
        <form onSubmit={handleSubmit(handleFormSubmit)} style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', height: '100%', width: '100%' }}>

          {/* === LEFT PANEL === */}
          <Box sx={{ flex: 1.1, p: { xs: 2.5, md: 4 }, overflowY: 'auto' }}>
            ...
            <Stack spacing={2.5} sx={{ mt: 1 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6} md={3}>
                  <Controller
                    ...
                    render={({ field }) => (
                      <TextField
                        {...field} select fullWidth label="Blood Group" disabled={!isEditing}
                        InputProps={{
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {['Male', 'Female', 'Other'].map(g => (
                    <QuickChip key={g} label={g} selected={currentGender === g} disabled={!isEditing} onClick={() => setValue('gender', g)} />
                  ))}
                </Box>
              </Box>
              ...
              <Box sx={{ bgcolor: '#fff5f5', p: 2, borderRadius: 2, border: '1px solid #ffebee', opacity: !isEditing ? 0.7 : 1 }}>
                ...
                <Grid container spacing={1.5}>
                  ...
                  <Grid item xs={12} sm={5}>
                    <TextField fullWidth size="small" placeholder="Relation / Phone" disabled={!isEditing} sx={{ bgcolor: 'white' }} {...register("emergencyRelation")} />
                  </Grid>
                </Grid>
              </Box>
              ...
              <TextField
                select fullWidth label="Assign Doctor" disabled={!isEditing} defaultValue=""
              ...
              >
                {doctorList && doctorList.length > 0 ? (
                  ...
                  <MenuItem disabled value=""><em>No Doctor Available</em></MenuItem>
                )}
              </TextField>
          ...
          {/* === RIGHT PANEL === */}
          <Box sx={{ flex: 1, bgcolor: '#f8fafc', p: { xs: 2.5, md: 4 }, borderLeft: isMobile ? 'none' : '1px solid #eee', overflowY: 'auto' }}>
            ...
            <Stack spacing={3} sx={{ mt: 1 }}>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete
                    ...
                    options={activeDictionary.concerns} // DYNAMIC CONCERNS
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Autocomplete
                    {...field} multiple disabled={!isEditing} 
                    options={activeDictionary.conditions} // DYNAMIC MEDICAL CONDITIONS
                    value={field.value || []}
                    onChange={(_, data) => field.onChange(data)}
                    freeSolo
              ...
              <Box>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={6}><UploadButton label="Photo" icon={<CameraAltIcon />} disabled={!isEditing} /></Grid>
                  <Grid item xs={12} sm={6}><UploadButton label="Scans/X-Ray" icon={<InsertDriveFileIcon />} disabled={!isEditing} /></Grid>
                </Grid>
              </Box>

              <TextField
                fullWidth multiline rows={3} label="Chief Notes" disabled={!isEditing}
      ...
      {/* AI CONSENT DIALOG COMMENTED OUT 
